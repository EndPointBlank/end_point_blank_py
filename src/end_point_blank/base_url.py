...
def resolve(environ: Any, trust_proxy_headers: bool = True) -> Dict[str, Any]:
    ...
    proxied = any(v is not None for v in (forwarded_proto, forwarded_host, forwarded_port))

    host_part, authority_port = _split_authority(
        forwarded_host or environ.get("HTTP_HOST") or environ.get("SERVER_NAME")
    )

    scheme = _clean_scheme(forwarded_proto or (None if proxied else environ.get("wsgi.url_scheme")))
...
def _clean_host(value: Any) -> Optional[str]:
    ...
    if not host:
        return None
    return host if (_HOSTNAME.match(host) or _IPV6.match(host)) else None
